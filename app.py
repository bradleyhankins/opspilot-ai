...
def best_manager_move(diagnosis: dict) -> str:
    if diagnosis["primary_bottleneck"] == "Demo Rate": return "Tighten confirmation and appointment-setting standards."
    if diagnosis["primary_bottleneck"] == "Close Rate": return "Run objection roleplay and review unsold demos."
    if diagnosis["primary_bottleneck"] == "Average Sale": return "Audit scopes for missed upgrades and add-ons."
    if diagnosis["primary_bottleneck"] == "NSLI": return "Review lead allocation and protect stronger lead sources."
...
def direction(value: float, is_money: bool = False, is_pct: bool = False) -> str:
    ...
    if is_money: return f"{arrow} {money(abs(value))}"
    if is_pct: return f"{arrow} {abs(value):.1%}"
...
html_card("Portfolio Skills Shown", "<ul><li>AI-enhanced manager brief with rules-based fallback</li><li>CSV intake and data validation</li><li>KPI calculation and target comparison</li><li>Rules-based operational diagnosis</li><li>Trend comparison logic</li><li>User-friendly PDF manager reporting</li></ul>", "coach-card")
