...
def build_manager_report(metrics, targets, diagnosis, priorities, health, period_change) -> str:
...
html_card("Executive Summary", f"<p>The team generated <strong>{money(metrics['total_revenue'])}</strong> from <strong>{metrics['total_sales']:,.0f} sales</strong> on <strong>{metrics['total_leads']:,.0f} leads issued</strong>.</p><ul><li><strong>Top revenue producer:</strong> {diagnosis['best_rep']['Rep']} at {money(diagnosis['best_rep']['Revenue'])}</li><li><strong>Rep to review:</strong> {diagnosis['review_rep']['Rep']} based on lowest NSLI</li><li><strong>Strongest lead source:</strong> {diagnosis['best_source']['Lead Source']} at {money(diagnosis['best_source']['NSLI'])} NSLI</li><li><strong>Lead source to review:</strong> {diagnosis['review_source']['Lead Source']} at {money(diagnosis['review_source']['NSLI'])} NSLI</li></ul>")
...
report = build_manager_report(metrics, targets, diagnosis, priorities, health, period_change)
...
html_card("Portfolio Skills Shown", "<ul><li>CSV intake and data validation</li><li>KPI calculation and target comparison</li><li>Rules-based operational diagnosis</li><li>Trend comparison logic</li><li>Manager-ready reporting and exports</li></ul>", "coach-card")
